...
| `outputCount` | 1~20 | 仅 `TXT2IMG` / `IMG2IMG` 生效，默认 1 |
...
| 精细化换装 | `VIRTUAL_TRY_ON_PRO` | `garment_reference`/`accessory_collage`/`scene_reference`/`pose_reference` |
| 姿势参考 | `POSE_TRANSFER` / `POSE_TRANSFER_PRO` | `pose_reference` |
| 姿势裂变 | `POSE_DUPLICATION` | `image` |
| 智能重绘 | `IMAGE_CLEANUP` | `image` |
| 高清放大 | `UPSCALE`（批量就多传几张） | `image` |
| 图片抠图 | `BACKGROUND_REMOVAL` | `image` |
| 换背景 | `BACKGROUND_REPLACEMENT` | `model`/`image` + `background`/`scene_reference` |
...
| 细节增强 | `DETAIL_ENHANCEMENT` | `model` + `face`/`hand`/`top`/`pants`/`whole_body`/`mask` |
| 模特三视图 | `MODEL_THREE_VIEW` | `model` + `garment_front` + `garment_back` |
| 编辑元素 | `IMAGE_LAYER_SPLIT` | `image` |
| 加噪点 / 追色 | `IMAGE_GRAIN` / `IMAGE_COLOR_RESTORE` | `image` |
...
**当前不可用**：对话生图 / 详情图生成（`IMAGE_CHAT`、`DETAIL_IMAGE_CHAT`）走另一套内核，Integration API 尚未开放，CLI 创建不了 —— 用户问起时如实说明，别拿 `IMG2IMG` 冒充。
